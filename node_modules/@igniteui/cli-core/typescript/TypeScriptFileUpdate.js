...
class TypeScriptFileUpdate {
    ...
    constructor(targetPath) {
        ...
        this.componentMetaTransformer = (context) => (rootNode) => {
            const visitComponent = (node) => {
                ...
                if (node.kind === ts.SyntaxKind.ObjectLiteralExpression &&
                    ...
                    const newProps = [];
                    const importDeps = this.ngMetaEdits.imports;
                    importsExpr = ts.factory.createArrayLiteralExpression(importDeps.map(x => TypeScriptUtils_1.TypeScriptUtils.createIdentifier(x.name)));
            ...
            const visitCondition = (node) => {
                return node.kind === ts.SyntaxKind.CallExpression &&
                    node.parent && node.parent.kind === ts.SyntaxKind.Decorator &&
                    node.expression.getText() === "Component";
            };
     ...
     */
    addChildRoute(filePath, linkPath, linkText, parentRoutePath, routesVariable = DEFAULT_ROUTES_VARIABLE, lazyload = false, routesPath = "", root = false, isDefault = false) {
        this.addRouteModuleEntry(filePath, linkPath, linkText, routesVariable, parentRoutePath, lazyload, routesPath, root, isDefault);
    }
    /**
     * Create configuration object for a component and add it to the `Routes` array variable.
     * Imports the first exported class and finalizes the file update (see `.finalize()`).
     ...
     * @param routesVariable Name of the array variable holding routes
     */
    addRoute(filePath, linkPath, linkText, routesVariable = DEFAULT_ROUTES_VARIABLE, lazyload = false, routesPath = "", root = false, isDefault = false) {
     ...
     */
    addStandaloneImport(dep, variables) {
        const copy = {
            import: this.asArray(dep.import, variables),
            provide: this.asArray(dep.provide, variables)
        };
     ...
     */
    addAppConfigProvider(dep, configVariable = DEFAULT_APPCONFIG_VARIABLE) {
        let providers = this.asArray(dep.provide, {});
        const transformer = (context) => (rootNode) => {
            const conditionalVisitor = (node) => {
                if (node.kind === ts.SyntaxKind.ArrayLiteralExpression &&
                    node.parent.kind === ts.SyntaxKind.PropertyAssignment &&
                    node.parent.name.getText() === "providers") {
                    const array = node;
                    const nodes = ts.visitNodes(array.elements, visitor);
                    const alreadyProvided = nodes.map(x => TypeScriptUtils_1.TypeScriptUtils.getIdentifierName(x));
            ...
            const visitCondition = (node) => {
                return node.kind === ts.SyntaxKind.VariableDeclaration &&
                    node.name.getText() === configVariable &&
                    node.type.getText() === "ApplicationConfig";
            };
    ...
    addRouteModuleEntry(filePath, linkPath, linkText, routesVariable = DEFAULT_ROUTES_VARIABLE, parentRoutePath, lazyload = false, routesPath = "", root = false, isDefault = false) {
        let className;
        const fileSource = TypeScriptUtils_1.TypeScriptUtils.getFileSource(filePath);
        const relativePath = Util_1.Util.relativePath(this.targetPath, filePath, true, true);
        className = TypeScriptUtils_1.TypeScriptUtils.getClassName(fileSource.getChildren());
            ...
            const parentRouteVisitor = (node) => {
                if (node.kind === ts.SyntaxKind.ObjectLiteralExpression) {
                    ...
                    if (!childrenProperty) {
                        ...
                        childrenProperty =
                            ts.factory.updatePropertyAssignment(childrenProperty, childrenPropertyName, ts.factory.createArrayLiteralExpression([...newArrayValues]));
                        existingProperties
                            .splice(index, 1, childrenProperty);
                    }
                    return ts.factory.updateObjectLiteralExpression(currentNode, existingProperties);
                }
                else {
                    return ts.visitEachChild(node, conditionalVisitor, context);
                }
    ...
    createRouteEntry(linkPath, className, linkText, lazyload = false, routesPath = "", root = false, isDefault = false) {
        const routePath = ts.factory.createPropertyAssignment("path", ts.factory.createStringLiteral(linkPath));
        if (isDefault) {
            const routeRedirectTo = ts.factory.createPropertyAssignment("redirectTo", ts.factory.createStringLiteral(routesPath));
            const routePathMatch = ts.factory.createPropertyAssignment("pathMatch", ts.factory.createStringLiteral("full"));
        ...
        if (lazyload) {
            if (root) {
                routeComponent = ts
                    .factory
                    .createPropertyAssignment("loadChildren", ts.factory.createIdentifier(`() => import('${routesPath}').then(m => m.routes)`));
            }
            else {
                routeComponent = ts
                    .factory
                    .createPropertyAssignment("loadComponent", ts.factory.createIdentifier(`() => import('./${linkPath}/${linkPath}.component').then(m => m.${className})`));
            }
        }
        else {
